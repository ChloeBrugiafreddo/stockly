...
import { Pencil, Trash2, Plus, Minus, History } from 'lucide-react'
...
export function StockTable({ items, loading, onEdit, onDelete, onMovement, onHistory }: Props) {
  if (loading) return (
          ...
          <tbody>
            {items.map((item, i) => (
              <tr key={item._id} style={{
                ...
                <td style={{ padding: '12px 16px', color: 'var(--muted)' }}>{item.minimumStock}</td>
                <td style={{ padding: '12px 16px', color: 'var(--foreground)' }}>
                  {item.price > 0 ? `${item.price.toFixed(2)} €` : '—'}
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    <button onClick={() => onMovement(item, 'IN')} title="Ajouter du stock" style={{
                    ...
                    <button onClick={() => onMovement(item, 'OUT')} title="Retirer du stock" style={{
                      width: '30px', height: '30px', borderRadius: '8px',
                    ...
                    <button onClick={() => onHistory(item)} title="Historique des mouvements" style={{
                      width: '30px', height: '30px', borderRadius: '8px',
                    ...
                    <button onClick={() => onEdit(item)} title="Modifier" style={{
                      width: '30px', height: '30px', borderRadius: '8px',
                      border: '1px solid var(--card-border)', background: 'transparent',
                      color: 'var(--muted)', cursor: 'pointer',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
